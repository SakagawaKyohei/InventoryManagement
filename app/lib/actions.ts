...
export async function AddUser(user:Users) {
  try {
    ...
    await sql`
      INSERT INTO users (name,email,password,status, role, bank, stk, ngay_sinh,sdt, cccd,dia_chi, permission)
      VALUES (${user.name},${user.email}, ${user.password},${user.status}, ${user.role},${user.bank}, ${user.stk}, ${user.ngay_sinh},${user.sdt},${user.cccd},${user.dia_chi}, ${permissionss})
    `;
  } catch (error) {
    // If a database error occurs, return a more specific error.
    console.log(error)
